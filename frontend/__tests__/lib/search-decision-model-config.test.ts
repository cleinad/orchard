...
describe('search decision model config', () => {
  ...
  it('uses qwen direct as the primary decision model and openrouter as fallback when both are configured', async () => {
    vi.stubEnv('QWEN_API_KEY', 'qwen-key');
    vi.stubEnv('QWEN_DECISION_MODEL', 'qwen2.5-3b-instruct');
  ...
  it('keeps openrouter as primary when qwen is not configured', async () => {
    vi.stubEnv('SEARCH_PLANNER_API_KEY', 'openrouter-key');
    vi.stubEnv('SEARCH_PLANNER_BASE_URL', 'https://openrouter.ai/api/v1');
    vi.stubEnv('SEARCH_PLANNER_MODEL', 'Qwen/Qwen2.5-3B-Instruct');
  ...
  it('can force openrouter even when qwen is configured', async () => {
